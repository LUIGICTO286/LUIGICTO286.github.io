...
export const Modal: React.FC<ModalProps> = ({ isOpen, onClose, children, title, className }) => {
  ...
  return ReactDOM.createPortal(
    ...
    >
      <div
        className={`relative rounded-lg bg-white p-4 shadow-lg ${className || ''}`}
        onClick={(e) => e.stopPropagation()} // Prevent closing the modal when clicking inside
      >
        <button className="absolute right-4 top-2 text-4xl text-gray-500 hover:text-gray-700" onClick={onClose}>
          &times;
        </button>
